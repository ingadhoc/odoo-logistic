...
class account_automatic_reconcile(models.TransientModel):
    ...
    power = fields.Selection(
        [(p, str(p)) for p in range(2, 5)],
        'Power',
        required=True,
        help='Number of partial amounts that can be combined to find a balance point can be chosen as the power of the automatic reconciliation',
        default=2)
    reconciled = fields.Integer('Reconciled transactions',
                                readonly=True, default=_get_reconciled)
    unreconciled = fields.Integer(
        'Not reconciled transactions',
        readonly=True, default=_get_unreconciled)
            ...
            def check3(value, list1, list2, list1power, power):
                for i in range(len(list1)):
                    ...
                    if list1power == 1:
                        ...
                        del list1[i]
                        res = check3(
                            value + move[1], list1, list2, list1power - 1, power - 1)
                        list1[i:i] = [move]
                        if res:
                            x, y = res
                            x.append(move[0])
        ...
        while credits and debits and ok:
            ...
            if res:
                move_line_obj.reconcile(
                    cr, uid, res[0] + res[1], 'auto', writeoff_acc_id, period_id, journal_id, context)
                reconciled += len(res[0]) + len(res[1])
                credits = [(id, credit)
                           for (id, credit) in credits if id not in res[0]]
                debits = [(id, debit)
                          for (id, debit) in debits if id not in res[1]]
            else:
                ok = False
    ...
    def reconcile(self, cr, uid, ids, context=None):
        ...
        if not form.account_ids:
            raise Warning(
                _('User Error!'), _('You must select accounts to reconcile.'))
        for account_id in form.account_ids:
            ...
            if not allow_write_off:
                ...
                params += (max_amount,)
            # reconcile automatically all transactions from partners whose
            # balance is 0
            ...
            for partner_id in partner_ids:
                ...
                if line_ids:
                    ...
                    if allow_write_off:
                        move_line_obj.reconcile(
                            cr, uid, line_ids, 'auto', form.writeoff_acc_id.id, form.period_id.id, form.journal_id.id, context)
                    else:
                        move_line_obj.reconcile_partial(
                            cr, uid, line_ids, 'manual', context=context)

            # get the list of partners who have more than one unreconciled
            # transaction
            cr.execute(
                "SELECT partner_id "
                "FROM account_move_line "
            ...
            # filter?
            for partner_id in partner_ids:
                # get the list of unreconciled 'debit transactions' for this
                # partner
                cr.execute(
                    "SELECT id, debit "
                    "FROM account_move_line "
                ...
                # get the list of unreconciled 'credit transactions' for this
                # partner
                cr.execute(
                    "SELECT id, credit "
                    "FROM account_move_line "
                ...
                (rec, unrec) = self.do_reconcile(cr, uid, credits, debits, max_amount, power,
                                                 form.writeoff_acc_id.id, form.period_id.id, form.journal_id.id, context)
                reconciled += rec
                unreconciled += unrec

            # add the number of transactions for partners who have only one
            # unreconciled transactions to the unreconciled count
            partner_filter = partner_ids and 'AND partner_id not in (%s)' % ','.join(
        ...
        model_data_ids = obj_model.search(cr, uid, [(
            'model', '=', 'ir.ui.view'), ('name', '=', 'account_automatic_reconcile_view1')])
